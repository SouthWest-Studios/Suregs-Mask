º#include "DialogTriggerEntity.h"
...
void DialogTrigger::PlayDialog()
{
	...
	if ((played && !repeatDialog) || !played) {
		...
		for (item = dialogues.start; item != NULL; item = item->next)
		{
			...
			// Si el di�logo tiene un evento asociado, man�jalo
...
void DialogTrigger::SetDialogId(uint new_id) {
	dialogId = new_id;
	UpdateDialog(); // Actualizar el di�logo cuando cambia el id
}
...
void DialogTrigger::UpdateDialog() {
	...
	// Obtener el id del di�logo
	uint id = GetDialogId();

	// Buscar el di�logo con el id correspondiente
	ListItem<Dialog*>* item;
	Dialog* pDialog = nullptr;

	// Si el di�logo ha sido reproducido y no se debe repetir, o si no ha sido reproducido
	if ((played && !repeatDialog) || !played) {
		for (item = dialogues.start; item != NULL; item = item->next) {
			if (item->data->id == id) {
				pDialog = item->data;
	...
	// Si el di�logo ha sido reproducido y se debe repetir
	else if (played && repeatDialog) {
		for (item = dialoguesRepeat.start; item != NULL; item = item->next) {
			if (item->data->id == id) {
				pDialog = item->data;
