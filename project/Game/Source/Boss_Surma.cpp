...
bool Boss_Surma::Start() {
	...
	surma_melee_attack_fx = app->audio->LoadAudioFx("surma_melee_attack_fx");
	surma_melee_attackAlt_fx = app->audio->LoadAudioFx("surma_melee_attackAlt_fx");
	surma_melee_attackAlt2_fx = app->audio->LoadAudioFx("surma_melee_attackAlt2_fx");
	surma_charged_melee_attack_fx = app->audio->LoadAudioFx("surma_charged_melee_attack_fx");
...
void Boss_Surma::Fase1(float dt, iPoint playerPos)
{
	...
	if (!jugadorCerca && !realizandoCombo) {
		...
		if (!combo1Anim.HasFinished()) {
			...
			if (combo1Fx == false) {
				app->audio->PlayFx(surma_melee_attack_fx);
				combo1Fx = true;
			}
			...
			if (combo2Fx == false) {
				app->audio->PlayFx(surma_melee_attackAlt_fx);
				combo2Fx = true;
			}
			//PARTICULA ATAQUE FLOJO
			...
			if (combo3Fx == false) {
				app->audio->PlayFx(surma_melee_attackAlt2_fx);
				combo3Fx = true;
			}
			//PARTICULA ATAQUE FLOJO
			...
			if (jugadorCerca || currentAnimation == &cansadoAnim) {
				if (cargaAtaqueTimer.ReadMSec() <= cargaAtaqueMS) {
					...
					if (chargedAttackFx == false) {
						app->audio->PlayFx(surma_charged_melee_attack_fx);
						chargedAttackFx = true;
					}
