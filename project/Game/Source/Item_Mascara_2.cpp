...
Item_mascara_2::Item_mascara_2(EntityType type, int id, int ataque, int durabilidad, int magia, float peso)
	: type(type), ataque(ataque), durabilidad(durabilidad), magia(magia), peso(peso), Entity(EntityType::ITEM_MASCARA2)
{
	name = ("item_mascara_2");
}

Item_mascara_2::~Item_mascara_2() {}

bool Item_mascara_2::Awake() {
...
bool Item_mascara_2::Update(float dt)
{
	// L07 DONE 4: Add a physics to an item - update the position of the object from the physics.  

	b2Transform pbodyPos = pbody->body->GetTransform();
	position.x = METERS_TO_PIXELS(pbodyPos.p.x) - 83;
	position.y = METERS_TO_PIXELS(pbodyPos.p.y) - 63;
...
bool Item_mascara_2::PostUpdate()
{
	app->render->DrawTexture(texture, position.x, position.y, 0.2f);
	return true;
}

bool Item_mascara_2::CleanUp()
{
