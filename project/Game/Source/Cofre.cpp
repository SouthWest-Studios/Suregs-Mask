			...
			if (app->input->GetButton(CONFIRM) == KEY_DOWN && !app->scene_pueblo->GetRod()->fishing.rodReady  && !app->scene_pueblo_tutorial->GetRod()->fishing.rodReady)
			{
				if (tutorial) {
					...
					if (app->questManager->GetQuestLineIndex(1) == 4) {
                      ...
                      nota2->Start();
                      nota2->pbody->ctype = ColliderType::NOTA_TUTORIAL;
                      ...
                      nota3->Start();
                      nota3->pbody->ctype = ColliderType::NOTA_TUTORIAL;
                      ...
                      nota4->Start();
                      nota4->pbody->ctype = ColliderType::NOTA_TUTORIAL;
					...
					for (int i = 0; i < 2; ++i) {
						int randomIndex = rand() % 101; // Elige un �ndice aleatorio de la lista
						Entity* item = nullptr;
